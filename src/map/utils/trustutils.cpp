...
auto trustutils::BuildMimicTrust(CCharEntity* PMaster, uint32 altCharId) -> CTrustEntity*
{
    ...
    auto* PTrust = new CTrustEntity(PMaster, 0, IsPassiveTrust{ false });
    ...
    for (int i = static_cast<int>(xi::SkillType::DivineMagic); i <= static_cast<int>(xi::SkillType::BlueMagic); i++)
    {
        uint16 maxSkill = battleutils::GetMaxSkill((xi::SkillType)i, snapshot.mjob, snapshot.mlvl > 99 ? 99 : snapshot.mlvl);
        if (maxSkill != 0)
        {
        ...
        {
            uint16 maxSubSkill = battleutils::GetMaxSkill((xi::SkillType)i, snapshot.sjob, snapshot.mlvl > 99 ? 99 : snapshot.mlvl);
            if (maxSubSkill != 0)
            {
