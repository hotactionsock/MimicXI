...
auto CheckInstance(Scheduler& scheduler, MapConfig config) -> Task<void>
{
    ...
    // MimicXI: enforce the concurrent-instance cap (max 3 of the same content running at once)
    if (auto* PZoneInst = dynamic_cast<CZoneInstance*>(zoneutils::GetZone(data.instance_zone)))
    {
