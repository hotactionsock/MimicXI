...
CInstanceLoader::CInstanceLoader(uint32 instanceid, CCharEntity* PRequester)
{
    ...
    // No ZONE_TYPE::Instanced gate here: zone-layer instances (see docs/rift_system_design.md)
    // run inside ordinary, non-instanced zones, sharing the zone ID with regular players.
    if (!PZone)
    {
...
auto CInstanceLoader::LoadInstance() const -> CInstance*
{
    ...
    auto rset = db::preparedStmt("SELECT mobname, mobid, pos_rot, pos_x, pos_y, pos_z, "
                                 ...
                                 "WHERE instanceid = ? "
                                 // MimicXI: do not filter out (0,0,0) spawn points — custom instance content
                                 // (Tier Trials, Circuit, Proving Arms) stores all spawn_points at (0,0,0)
                                 // by design; the engine positions mobs within the instance at runtime.
