...
auto DetourNavMesh::findAnyRandomPosition() const -> Maybe<position_t>
{
    TracyZoneScopedS(12);

    DebugNavmesh("DetourNavMesh::findAnyRandomPosition (%u)", zoneID_);

    const auto filter = makeFilter();

    dtPolyRef  randomRef = 0;
    float      randomPt[3];
    const auto status = navMeshQuery_.findRandomPoint(
        &filter, []() -> float
        {
            return xirand::GetRandomNumber(1.0f);
        },
    ...
    if (dtStatusFailed(status))
    {
        ShowError("DetourNavMesh::findAnyRandomPosition error (%u): %s", zoneID_, detourStatusString(status));
        return std::nullopt;
    }
