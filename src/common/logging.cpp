...
void logging::InitializeLog(const std::string& serverName, const std::string& logFile, bool appendDate)
{
    ...
    {
        // Synchronous, not spdlog::async_logger: crash logs must be complete up to the
        // exact point of failure, which an async background-thread logger cannot guarantee.
        auto logger = std::make_shared<spdlog::logger>(std::string(logNames[i]), sinks.begin(), sinks.end());
