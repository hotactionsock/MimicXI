...
-- pool applies them at award time.  Because the lot window only shows the
-- base item (no exdata), augments are announced via system message so
-- players know what they are lotting on.
--
...
-----------------------------------
-- Build a human-readable augment string, e.g. "DEX+3 / Accuracy+10"
-----------------------------------
...
xi.caskets.mimic.mobCallbacks.onMobDeath = function(mob, player, isKiller, noKillIncrement)
    ...
    -- Announce augmented items so players know what they are lotting on.
    -- The lot window only displays the base item ID; augments are invisible
    -- until the winner actually receives the item.
    if hasPool then
