...
describe('Treasure Hunter', function()
    ...
    it('equals TH0 for a naked THF75 with no missions completed', function()
        local freshPlayer = xi.test.world:spawnPlayer({ zone = xi.zone.KUFTAL_TUNNEL, job = xi.job.THF, level = 75 })
        freshPlayer.assert:hasModifier(xi.mod.TREASURE_HUNTER, 0)
    end)

    it('equals TH1 after completing the Shadow Lord mission', function()
        local missionPlayer = xi.test.world:spawnPlayer({ zone = xi.zone.KUFTAL_TUNNEL, job = xi.job.THF, level = 75 })
