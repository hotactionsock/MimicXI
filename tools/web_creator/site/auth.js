...
(function () {
  ...
  // Calls through `MimicAuth.api` (rather than the local `api`) so that
  // demo.js — which monkey-patches MimicAuth.api — can intercept it.
  async function requireAuth() {
    const r = await window.MimicAuth.api("/api/characters");
    if (r.status === 401) {
